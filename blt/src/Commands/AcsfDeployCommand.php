...
/**
 * Defines commands in the "acsf-deploy:*" namespace.
 *
 ...
 */
class AcsfDeployCommand extends BltTasks {
  ...
  /**
   * Builds separate artifact and pushes to git remotes defined in project.yml.
   *
   * @command acsf-deploy
   */
  ...
  protected function checkoutLocalDeployBranch() {
    $this->taskExecStack()
      ->dir($this->deployDir)
            // Create new branch locally.We intentionally use
            // stopOnFail(FALSE) in case the branch already exists.
            // `git checkout -B` does not seem to work as advertised.
  ...
  protected function mergeUpstreamChanges() {
    ...
    $this->taskExecStack()
      ->dir($this->deployDir)
            // This branch may not exist upstream, so we
            // do not fail the build if a merge fails.
      ->stopOnFail(FALSE)
  ...
  protected function buildCopy() {
    ...
    $this->setSitesFilePermissions(0777);
    ...
    $this->setSitesFilePermissions(0755);
    ...
    $this->taskFilesystemStack()
      ...
      ->copy(
            $this->getConfigValue('repo.root') . '/build/core/files/.gitignore',
  ...
  /**
   * Sets permissions for sites directories.
   */
  protected function setSitesFilePermissions($perms) {
    $taskFilesystemStack = $this->taskFilesystemStack();
    $site_dir = $this->getConfigValue('docroot') . '/sites/default';
    $taskFilesystemStack->chmod($site_dir, $perms);
    $taskFilesystemStack->setVerbosityThreshold(VerbosityThresholdInterface::VERBOSITY_VERBOSE);
    $taskFilesystemStack->run();
  }
  ...
  protected function commit() {
    ...
    $this->taskExecStack()
      ->dir($this->deployDir)
      ->exec("git add -A")
      ->exec("git add --force {$this->deployDir}/docroot/profiles/custom/webny/themes/custom/webny_theme/css")
      ->exec("git add --force {$this->deployDir}/docroot/profiles/custom/webny/themes/custom/webny_theme/styleguide")
  ...
  /**
   * If we are using ACSF, run the ACSF Deploy command deploy:acsf:init.
   */
  protected function deployAcsfInit() {
    if ($this->getConfigValue('acsf')) {
      $this->invokeCommand('deploy:acsf:init');
    }
  }

  /**
   * Build a subsidiary makefile shipped with profile.
   *
   * @throws \Exception
   */
  protected function buildMakeFile() {
    if ($this->getConfigValue('project.profile.contrib')) {
      $this->say("Building makefile for {$this->getConfigValue('project.profile.name')} ...");
      $profile_directory = $this->deployDir . '/docroot/profiles/contrib/' . $this->getConfigValue('project.profile.name');
      $result = $this->taskDrush()
        ->drush("make")
        ->arg("$profile_directory . '/drupal-org.make'")
      ...
      if (!$result->wasSuccessful()) {
        throw new BltException("The makefile could not be generated.");
      }
    }
  }
   ...
   *
   * @command acsf-deploy:drupal:install
   */
